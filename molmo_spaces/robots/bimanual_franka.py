"""Bimanual Franka robot implementation (two FR3 arms with Robotiq 2F-85 grippers)."""
...
from molmo_spaces.kinematics.bimanual_franka_kinematics import BimanualFrankaKinematics
...
if TYPE_CHECKING:
    ...
    from molmo_spaces.configs.robot_configs import BimanualFrankaConfig


class BimanualFrankaRobot(Robot):
    """Bimanual Franka robot (two 7-DOF FR3 arms with Robotiq 2F-85 grippers)."""
    ...
    def __init__(
        ...
        self._controllers = {
            ...
            "left_gripper": gripper_controller_cls(
                self._robot_view.get_move_group("left_gripper")
            ),
    ...
    def state_dim(self) -> int:
        return 14  # Two 7-DOF arms

    def action_dim(self, move_group_ids: list[str]):
        return sum(
            self._robot_view.get_move_group(mg_id).n_actuators for mg_id in move_group_ids
        )

    def get_arm_move_group_ids(self) -> list[str]:
        """Bimanual Franka has two independent arms."""
    ...
    def robot_model_root_name() -> str:
        """The root body name in fr3_duo.xml."""
        return "base"
    ...
    def add_robot_to_scene(
        ...
        # Attach the bimanual robot model
